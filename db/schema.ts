...
export const humanStyleProfiles = mysqlTable("human_style_profiles", {
  ...
  // 2026-09-21 产品确认：字段正式改名，原 bone_width 实际表达的是"肩线走向"，
  // 跟"肩宽"语义容易混淆，改名成本最低的阶段就应该改，不等以后批量整理。
  // 取值范围暂保留中文（溜/匀/直）——Mapping Matrix 文档建议的英文值
  // （sloped/natural/straight）涉及改动前端 BodyTestPage.tsx 的题目选项 id
  // 和已有数据的值迁移，范围更大，这次先只做"字段名"这一层迁移，
  // 值域是否也要改成英文，需要产品确认后再单独排期。
  shoulderLineDirection: mysqlEnum("shoulder_line_direction", ["溜", "匀", "直"]),
